...
import { collection, query, getDocs, where, Timestamp, doc, setDoc } from 'firebase/firestore';
...
export default function EarningsAdmin() {
  ...
  useEffect(() => {
    async function fetchAppointments() {
      ...
      try {
        ...
        calculateEarnings(appointmentsList);
      } catch (e) {
  ...
  const calculateEarnings = (appointmentsList: Appointment[]) => {
  ...
  // No longer needed after removing period filter
  ...
  return (
    <div className="p-6">
      ...
      {/* Base Fee Configuration and Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100 hover:shadow-md transition-all">
          ...
          <p className="text-3xl font-bold text-[#14396D]">{formatCurrency(totalEarnings)}</p>
          <p className="text-sm text-gray-500 mt-2">All non-cancelled appointments</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100 hover:shadow-md transition-all">
          <div className="flex items-center mb-2">
            ...
            <h3 className="text-gray-600 text-sm font-medium">Default Base Fee</h3>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <div className="relative rounded-md shadow-sm w-full">
              ...
              <input 
                ...
                className="w-full pl-12 pr-12 p-2 border rounded text-xl font-bold text-green-600 focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
                min="0"
                required
              />
            </div>
          ...
          <p className="text-sm text-gray-500 mt-2">Default fee for new appointments</p>
          {success && (
            <div className="mt-2 p-2 bg-green-50 text-green-800 text-xs rounded-md border border-green-200 flex items-center">
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100 hover:shadow-md transition-all">
          <div className="flex items-center mb-2">
            ...
            <h3 className="text-gray-600 text-sm font-medium">Average Fee</h3>
          </div>
          <p className="text-3xl font-bold text-purple-600">
            {(() => {
              ...
              if (validAppointments.length > 0) {
                return formatCurrency(Math.round(totalEarnings / validAppointments.length));
              }
              return 'N/A';
            })()}
          </p>
          <p className="text-sm text-gray-500 mt-2">Average fee per appointment</p>
        </div>
      </div>
      
      {/* Earnings Over Time Chart */}
      <div className="bg-white p-6 rounded-lg shadow mb-6 border border-gray-100">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">Monthly Earnings Trend</h2>
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <p>Loading chart data...</p>
          </div>
        ) : earningsByPeriod.labels.length > 0 ? (
          <div className="h-64">
            <Line
              data={{
                labels: earningsByPeriod.labels,
                datasets: [
                  {
                    label: 'Earnings',
                    data: earningsByPeriod.data,
                    borderColor: '#3373FF',
                    backgroundColor: 'rgba(51, 115, 255, 0.1)',
                    tension: 0.2,
                    fill: true,
                  },
                ],
